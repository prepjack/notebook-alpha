...
async function removeTopicContent() {
    ...
    try {
        ...
        // NEW (2026-09-10): flags this topic's Content_Core rows with
        // orphaned_at / orphan_reason = "content_removed" — a topic
        // whose content was cleared but whose Nodes row (and Drive
        // folder) still exist, so it would never otherwise show up in
        // the review workflow used for deleted topics or missing Drive
        // folders. Fire-and-forget is fine here (unlike the index-term
        // unlink above): the worst case is just a missed review flag,
        // not a broken link visible on the site, so it doesn't need the
        // same verify+retry+alert treatment.
        fetch(GOOGLE_SHEET_API, {
            method: "POST",
            mode: "no-cors",
            body: JSON.stringify({ action: "flag_content_removed", node_id: selectedTopicNode.id })
        }).catch(err => console.warn("Flagging removed content as orphaned failed:", err));
