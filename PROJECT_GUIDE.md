...
- `revision.html` + `js/revision.js` — the Revision page: every topic with flashcards due, from review progress (see "Revision page" below). Uses the same `css/style.css`, header and footer pattern as the Index page.
...
## Revision page — Stage 1 (2026-09-20)

Answers the original wish "the site should tell me what to revisit". Stage 1 is deliberately small: it needs NO card text and NO new synced data.

- **Where:** header link "Revision · N" on `index.html` (next to ⟳ Sync; `#revision-link` / `#revision-badge`), the "Revise" button of the post-read strip (opens `revision.html?topic=<currentTopicId>` in a NEW tab and highlights that topic), and a footer link on each page (added to `index.html` and `revision.html`; `index-directory.html` and `mcq.html` need the same one line: `<a href="revision.html">Revision</a>` in `.app-footer-nav`).
- **Page (`revision.html`, `js/revision.js`):** body classes `index-directory-page revision-page` (reuses the shared sticky header rule), header with "← Home" and the ⟳ Sync button, one `.panel`. It declares its own `GOOGLE_SHEET_API` (each standalone page does; `progress-sync.js` reads that global). Script order: `data/study-data.js`, `progress-sync.js`, `flashcards.js`, `revision.js`.
- **Data:** due counts come from `Flashcards.getDueSummary()` (progress in `localStorage`). A card id is `topicId::hash`, so the topic id is the part before the last `::`. Topic names and paths come from the notebook tree loaded like `js/index-directory.js` does: the full Google Sheets API dump (`nodes` only are used) -> `data/study-data.json` -> `window.STUDY_DATA_FALLBACK`. (The full dump is heavy; a lighter `get_nodes` action in `Code.gs` is a possible later optimisation.)
- **Due rules:** only cards that have been reviewed at least once (an entry exists). Due = `dueDate <= today`; overdue = before today; "next 7 days" = the following week. The badge counts overdue + due today only; NEW (never reviewed) cards are not counted and not listed (they live inside their topic).
- **Ghost cards:** every time a topic's deck loads, `setContext()` stores that topic's CURRENT card ids (ids only, no text) under `localStorage["flashcards:decks:v1"]`. `getDueSummary()` ignores progress entries whose id is not in a topic's known deck, so a rewritten/edited deck can't leave phantom due cards in the badge. Topics never opened on this browser count all their entries until opened once. A deck is never recorded as empty.
- **Page contents:** summary line ("N cards due now, across M topics" with overdue / due-today chips), a note for the highlighted topic when it has nothing due or no progress, "Due now" (most overdue first) and "Coming up in the next 7 days" groups; each row = topic title, parent path, chips, "Open topic" -> `index.html?openNode=<id>` (existing deep link; the Flashcard button there shows only due cards). A topic id missing from the tree is listed as "Topic not found" without a link. Empty states: "No revision data yet…" / "Nothing is due right now."
- **Live refresh:** `writeStore()` in `flashcards.js` dispatches `flashcards-progress-changed` (own answers and progress merged in by a sync), which refreshes the badge and the page. A browser that has synced before syncs shortly after load; a brand-new browser sees the first-run banner.
- **Not in Stage 1 (Stage 2, only if it proves needed):** reviewing cards from several topics in one session. That needs card text for topics that aren't open: either cache text in `localStorage`/IndexedDB (per browser, ~5 MB limit, only topics opened there) or fetch each topic's deck from Drive (one Apps Script call per topic, needs internet). Drive stays the source of truth either way.
- Tested in Node/jsdom only (counts, ghost filtering, paths, chips, deep links, highlight notes, empty states, live refresh, API-down fallback, badge, Revise button).

## Roadmap & open decisions (as of 2026-09-20)

**Done:** flashcards (bilingual deck, Leitner, unlimited flip, modal), flashcard prompt + popup button, companion-file support in `Code.gs`, progress sync + backup, Revision page Stage 1 (header badge + due list + Revise button; not yet confirmed live). Deployed and confirmed by the owner: `Code.gs` (new version), `app.js`, `flashcards.js`, `style.css` up to the flashcard work. The sync work (`progress-sync.js`, `index.html`, the PROGRESS SYNC part of `Code.gs`, first with a key, then key-less on 2026-09-20) was delivered after that and is not yet confirmed live; if it was deployed in its key version, redeploy `Code.gs` as a New version and replace the site files.

**Next, in the agreed order:**
1. **Revision page Stage 2 (optional):** a cross-topic review session ("Review all due"). Stage 1 (list + badge + Revise button) is built, see the Revision page section; decide on Stage 2 after using it for a while, because it needs card text for topics that aren't open. Also pending: a small question from the owner side, whether the home page should also get a "Due today" panel (default: no, header link only).
...
**Deferred / stubs:** the Take Test button (inert), right-click index scopes, Google Sign-in for a public multi-user version, cleanup of progress entries for deleted topics.
